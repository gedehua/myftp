...
public class FtpHandler extends Thread{
	    ...
	    String reply;
		BufferedReader ctrlInput;//返回报告
	    BufferedWriter ctrlOutput;
	    ...
	    public void run()
	    {
	        ...
	        try
	        {
				ctrlInput = new BufferedReader
						(new InputStreamReader(ctrlSocket.getInputStream(),"GBK"));
	           	ctrlOutput =  new BufferedWriter
						(new OutputStreamWriter(ctrlSocket.getOutputStream(),"GBK"));
	            state  = FtpState.FS_WAIT_LOGIN;          //0
	            boolean finished = false;
				ctrlOutput.write("220 准备为您服务,你是当前第 " +id+ " 个登入者!\n");
				ctrlOutput.flush();
				ctrlInput.readLine();
				ctrlOutput.write("\nPlease enter your account! \r\n");
				ctrlOutput.flush();
	            while(!ctrlSocket.isClosed())
	            {
	                ...
	                else
	                {
	                    ...
	                    switch(state)                    //用户状态开关
	                    {
	                        case FtpState.FS_WAIT_LOGIN:   //0
	                                finished = commandUSER();
	                                break;
	                        case FtpState.FS_WAIT_PASS:    //1
	                                finished = commandPASS();
	                                break;
	                        case FtpState.FS_LOGIN:        //2
	                        {
	 ...
	 */
	    ...
	    boolean commandLIST()//文件和目录的列表
	    {
	        try
	        {
	            ...
	            dataSocket = new Socket(remoteHost,remotePort,InetAddress.getByName("127.0.0.1"),20);

	            BufferedWriter dout =  new BufferedWriter
						(new OutputStreamWriter(dataSocket.getOutputStream(),"GBK"));

	            if(param.equals("") || param.equals("LIST"))
	            {
	                ctrlOutput.write("150 文件状态正常,ls以 ASCII 方式操作\n");
	                ...
	                for(int i =0; i<dirStructure.length;i++)
	                {
	                    ...
	                    dout.write(fileType + dirStructure[i]);//(fileType+dirStructure[i]);
						dout.newLine();
						dout.flush();
	                }
	            }
	    ...
	    boolean commandRETR()
	    {
	        ...
	        else
	        {
	             if(type==FtpState.FTYPE_IMAGE)                //bin
	            {
	                try
	                {
	                    ctrlOutput.write("150 文件状态正常,以二进治方式打开文件:  "+ requestfile+"\n");
	                    dataSocket = new Socket(remoteHost,remotePort,InetAddress.getByName("127.0.0.1"),20);
	            ...
	            if(type==FtpState.FTYPE_ASCII)//ascII
	            {
	                  try
	                {
	                    ctrlOutput.write("150 Opening ASCII mode data connection for "+ requestfile+"\n");
	                    dataSocket = new Socket(remoteHost,remotePort,InetAddress.getByName("127.0.0.1"),20);
	                    BufferedReader  fin = new BufferedReader(new FileReader(requestfile, StandardCharsets.UTF_8));
						BufferedWriter dataOutput  =  new BufferedWriter
								(new OutputStreamWriter(dataSocket.getOutputStream(),"GBK"));
	                    String s;
	    ...
	    boolean commandSTOR()
	    {
	        ...
	        if(type == FtpState.FTYPE_IMAGE)//bin
	        {
	            try
	            {
	                ctrlOutput.write("150 Opening Binary mode data connection for "+ requestfile+"\n");
					ctrlOutput.flush();
	                dataSocket = new Socket(remoteHost,remotePort,InetAddress.getByName("127.0.0.1"),20);
	        ...
	        if(type == FtpState.FTYPE_ASCII)//ascII
	        {
	            try
	            {
	                ...
	                BufferedReader dataInput = new
							BufferedReader(new InputStreamReader(dataSocket.getInputStream(),"gbk"));
	                String line;
